...
describe('Model Predictions Integration Tests', () => {
  ...
  describe('Scenario-based Predictions', () => {
    ...
    it('should adjust strategy based on position', async () => {
      ...
      const earlyAction = await ai.getOptimalAction(
        playerCards,
        [],
        earlyPositionContext
      );
      const lateAction = await ai.getOptimalAction(
        playerCards,
        [],
        latePositionContext
      );
    ...
    it('should adapt to different board textures', async () => {
      ...
      const dryBoardAction = await ai.getOptimalAction(
        playerCards,
        dryBoard,
        gameContext
      );
      const wetBoardAction = await ai.getOptimalAction(
        playerCards,
        wetBoard,
        gameContext
      );
  ...
  describe('Performance Benchmarks', () => {
    ...
    it('should handle batch predictions efficiently', async () => {
      ...
      const predictions = await Promise.all(
        scenarios.map(s =>
          ai.getOptimalAction(s.playerCards as Card[], [], s.gameContext)
        )
      );
      const endTime = performance.now();
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle heads-up vs multi-way pots differently', async () => {
      ...
      const headsUpAction = await ai.getOptimalAction(
        playerCards,
        [],
        headsUpContext
      );
      const multiWayAction = await ai.getOptimalAction(
        playerCards,
        [],
        multiWayContext
      );
