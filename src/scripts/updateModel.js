...
class ModelUpdater {
  ...
  async checkDataRequirements() {
    try {
      ...
      if (stats.total_hands < newHandsThreshold) {
        console.log(`Only ${stats.total_hands} total hands, need at least ${newHandsThreshold}`)
        return false
      }
  ...
  async trainWithNewData() {
    try {
      ...
      // Validate model
      const validation = await this.trainer.validateModel(modelId, trainingExamples.slice(0, 100))
      ...
      if (validation.accuracy > currentAccuracy) {
        console.log(`New model accuracy (${validation.accuracy}) is better than current (${currentAccuracy})`)
        await this.trainer.updateModelConfiguration(modelId)
        await this.logSuccessfulUpdate(modelId, validation.accuracy)
      } else {
        console.log(`New model accuracy (${validation.accuracy}) is not better than current (${currentAccuracy}), keeping current model`)
      }
  ...
  async validateAndDeploy() {
    try {
      ...
      const validationResult = await this.performDeploymentValidation(config.modelId)
  ...
  async performDeploymentValidation(modelId) {
    try {
      ...
      const testScenarios = [
        {
          playerCards: [{ rank: 'A', suit: 'h' }, { rank: 'K', suit: 'h' }],
          boardCards: [{ rank: 'Q', suit: 'h' }, { rank: 'J', suit: 'h' }, { rank: 'T', suit: 'h' }],
          expectedAction: 'raise'
        },
        {
          playerCards: [{ rank: '2', suit: 'c' }, { rank: '7', suit: 'd' }],
          boardCards: [{ rank: 'A', suit: 'h' }, { rank: 'K', suit: 's' }, { rank: 'Q', suit: 'd' }],
          expectedAction: 'fold'
        }
      ]
...
async function main() {
  ...
  try {
    ...
    if (args.includes('--scheduled')) {
      ...
      console.log('Model updater running in scheduled mode. Press Ctrl+C to stop.')
