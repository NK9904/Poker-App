        ...
        messages: [
          {
            role: 'system',
            content:
              'You are an expert poker AI trained on professional player data. Analyze the given poker situation and provide the optimal action.',
          },
          {
            role: 'user',
...
Based on professional poker data, what is the optimal action in this situation?`;
  ...
  buildExpectedResponse(hand) {
    ...
    const betSize = hand.bet_size
      ? ` with a bet size of $${hand.bet_size}`
      : '';
  ...
  async createTrainingFile(trainingExamples) {
    ...
    console.log(
      `Created training file with ${trainingExamples.length} examples`
    );
    return trainingFile;
  }
  ...
  async validateModel(modelId, testData) {
    ...
    for (const testCase of testData.slice(0, 100)) {
      try {
        const response = await this.callFineTunedModel(
          modelId,
          testCase.messages[1].content
        );
        validationResults.push({
          expected: testCase.messages[2].content,
          actual: response,
          correct: this.evaluateResponse(
            response,
            testCase.messages[2].content
          ),
        });
      } catch (error) {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are an expert poker AI trained on professional player data.',
          },
          {
            role: 'user',
            content: prompt,
  ...
  async generateSyntheticData() {
    ...
    for (let i = 0; i < 10000; i++) {
      ...
      const boardCards = this.generateRandomCards(
