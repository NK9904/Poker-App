  ...
  async scrapeTournamentData(page, source) {
    ...
    const hands = await page.evaluate(() => {
      const handElements = document.querySelectorAll(
        '.hand-history, .tournament-hand'
      );
      return Array.from(handElements).map(element => {
        // Extract hand data from tournament pages
        const playerCards =
          element.querySelector('.player-cards')?.textContent || '';
        const boardCards =
          element.querySelector('.board-cards')?.textContent || '';
        const potSize = parseInt(
          element
            .querySelector('.pot-size')
  ...
  async scrapeLiveData(page, source) {
    ...
    const liveData = await page.evaluate(() => {
      const liveElements = document.querySelectorAll(
        '.live-hand, .live-action'
      );
      return Array.from(liveElements).map(element => {
        const playerCards = element.querySelector('.cards')?.textContent || '';
        const boardCards =
          element.querySelector('.community-cards')?.textContent || '';
        const potSize = parseInt(
          element.querySelector('.pot')?.textContent?.replace(/[^0-9]/g, '') ||
            '0'
        );
        const action =
          element.querySelector('.player-action')?.textContent || '';
  ...
  async scrapePlayerData(page, source) {
    ...
    const players = await page.evaluate(() => {
      ...
      return Array.from(playerElements).map(element => {
        ...
        const avgBetSize = parseFloat(
          element
            .querySelector('.avg-bet')
            ?.textContent?.replace(/[^0-9.]/g, '') || '0'
        );
  ...
  async scrapeStatsData(page, source) {
    ...
    const stats = await page.evaluate(() => {
      ...
      return Array.from(statElements).map(element => {
        const playerName = element.querySelector('.player')?.textContent || '';
        const vpip = parseFloat(
          element
            .querySelector('.vpip')
            ?.textContent?.replace(/[^0-9.]/g, '') || '0'
        );
        const pfr = parseFloat(
          element.querySelector('.pfr')?.textContent?.replace(/[^0-9.]/g, '') ||
            '0'
        );
        const af = parseFloat(
          element.querySelector('.af')?.textContent?.replace(/[^0-9.]/g, '') ||
            '0'
        );
