...
export class PokerEngine {
  ...
  evaluateHand(playerCards: Card[], boardCards: Card[] = []): HandEvaluation {
    ...
    if (allCards.length < 2) {
      return { rank: HandRank.HIGH_CARD, strength: 0, description: 'No hand', kickers: [] }
    }
  ...
  private performHandEvaluation(cards: Card[]): HandEvaluation {
    ...
    if (isFlush && isStraight && ranks[0] === 14) {
      return { rank: HandRank.ROYAL_FLUSH, strength: 1.0, description: 'Royal Flush', kickers: [] }
    }
    
    if (isFlush && isStraight) {
      return { rank: HandRank.STRAIGHT_FLUSH, strength: 0.95 + ranks[0] / 1000, description: 'Straight Flush', kickers: [] }
    }
    
    if (maxCount === 4) {
      return { rank: HandRank.FOUR_OF_A_KIND, strength: 0.85 + ranks[0] / 1000, description: 'Four of a Kind', kickers: ranks.slice(1) }
    }
    
    if (counts[0] === 3 && counts[1] === 2) {
      return { rank: HandRank.FULL_HOUSE, strength: 0.75 + ranks[0] / 1000, description: 'Full House', kickers: [] }
    }
    
    if (isFlush) {
      return { rank: HandRank.FLUSH, strength: 0.65 + ranks[0] / 1000, description: 'Flush', kickers: ranks.slice(1) }
    }
    
    if (isStraight) {
      return { rank: HandRank.STRAIGHT, strength: 0.55 + ranks[0] / 1000, description: 'Straight', kickers: [] }
    }
    
    if (maxCount === 3) {
      return { rank: HandRank.THREE_OF_A_KIND, strength: 0.45 + ranks[0] / 1000, description: 'Three of a Kind', kickers: ranks.slice(1) }
    }
    
    if (counts.filter(c => c === 2).length === 2) {
      return { rank: HandRank.TWO_PAIR, strength: 0.35 + ranks[0] / 1000, description: 'Two Pair', kickers: ranks.slice(2) }
    }
    
    if (maxCount === 2) {
      return { rank: HandRank.PAIR, strength: 0.25 + ranks[0] / 1000, description: 'Pair', kickers: ranks.slice(1) }
    }
    
    return { rank: HandRank.HIGH_CARD, strength: ranks[0] / 100, description: 'High Card', kickers: ranks.slice(1) }
  }
   ...
   */
  async calculateEquity(playerCards: Card[], boardCards: Card[] = [], iterations = 50000): Promise<EquityResult> {
    const cacheKey = this.getEquityCacheKey(playerCards, boardCards, iterations)
  ...
  private calculateEquitySync(playerCards: Card[], boardCards: Card[], iterations: number): EquityResult {
    let wins = 0, ties = 0, losses = 0
  ...
  private simulateRandomGame(playerCards: Card[], boardCards: Card[]): number {
    ...
    if (playerEval.strength > opponentStrength) return 1
    if (playerEval.strength === opponentStrength) return 0
    return -1
  }
  ...
  async calculateGtoStrategy(
    ...
    if (!this.worker) {
      return this.calculateGtoStrategySync(playerCards, boardCards, potSize, stackSize)
    }
  ...
  private calculateGtoStrategySync(
    ...
    if (handStrength < 0.3) {
      actions.push({ 
        ...
        reasoning: 'Weak hand, fold to avoid further losses' 
      })
      actions.push({ 
        action: 'call', 
        frequency: 0.2, 
        expectedValue: -potSize * 0.05, 
        reasoning: 'Moderate hand, call to see more cards' 
      })
      actions.push({ 
        action: 'raise', 
        frequency: 0.1, 
        sizing: potSize * 0.5, 
        expectedValue: potSize * 0.2, 
        ...
        actions.push({ 
          ...
          frequency: 0.1, 
          expectedValue: -potSize * 0.2, 
          reasoning: 'Decent hand, but fold to strong aggression' 
        })
        actions.push({ 
          action: 'call', 
          frequency: 0.6, 
          expectedValue: potSize * 0.1, 
          reasoning: 'Good hand, call to extract value' 
        })
        actions.push({ 
          action: 'raise', 
          frequency: 0.3, 
          sizing: potSize * 0.75, 
          expectedValue: potSize * 0.3, 
          reasoning: 'Strong hand, raise for value' 
        })
      } else {
        actions.push({ 
          ...
          frequency: 0.2, 
          expectedValue: potSize * 0.2, 
          reasoning: 'Very strong hand, call to trap' 
        })
        actions.push({ 
          action: 'raise', 
          frequency: 0.8, 
          sizing: potSize * 1.0, 
          expectedValue: potSize * 0.5, 
          reasoning: 'Monster hand, raise for maximum value' 
        })
      }
   ...
   */
  generateOptimalRanges(position: string): string[] {
    const ranges = {
      earlyPosition: [
  ...
  private getEquityCacheKey(playerCards: Card[], boardCards: Card[], iterations: number): string {
...
export const calculateHandStrength = (playerCards: Card[], boardCards: Card[] = []): number => {
  return pokerEngine.evaluateHand(playerCards, boardCards).strength
}

export const calculateQuickEquity = async (playerCards: Card[], boardCards: Card[] = []): Promise<number> => {
  const result = await pokerEngine.calculateEquity(playerCards, boardCards, 10000)
  return result.winRate
}

export const getHandDescription = (playerCards: Card[], boardCards: Card[] = []): string => {
