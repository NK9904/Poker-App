...
export function usePerformance() {
  ...
  const updatePerformanceData = useCallback(() => {
    ...
    const longTasks = metrics.filter(
      (m: PerformanceMetric) => m.name === 'long-task'
    ).length;
    const layoutShifts = metrics.filter(
      (m: PerformanceMetric) => m.name === 'layout-shift'
    ).length;
    const fpsMetrics = metrics.filter(
