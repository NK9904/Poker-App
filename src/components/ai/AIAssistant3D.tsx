...
function ActionCards({ actions, onActionSelect }: { 
  actions: PokerAction[], 
  onActionSelect: (action: PokerAction) => void 
}) {
  return (
...
// Main 3D Scene
function Scene({ analysis, isLoading, onActionSelect }: {
  analysis: AnalysisResult | null
  isLoading: boolean
  onActionSelect: (action: PokerAction) => void
}) {
  return (
    <>
      ...
      {analysis && analysis.actions.length > 0 && (
        <ActionCards actions={analysis.actions} onActionSelect={onActionSelect} />
      )}
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-xs text-gray-400">
              {isLoading ? 'Analyzing...' : analysis ? 'Ready' : 'Waiting for input'}
            </div>
          </div>
        </div>
      ...
      <div className="p-3 border-t border-gray-700/50 bg-gray-800/50">
        <div className="flex items-center justify-between text-xs text-gray-400">
          <div className="flex items-center gap-2">
            ...
            <span>
              {isLoading ? 'Processing' : analysis ? 'Analysis Complete' : 'Ready'}
            </span>
          </div>
