...
class SetupWizard {
  ...
  async setupDatabase() {
    ...
    try {
      ...
      const { PokerDataScraper } = await import(
        '../src/scripts/scrapePokerData.js'
      );
      ...
      console.warn(
        '⚠️  Database setup failed, will retry later:',
        error.message
      );
    }
  }
  ...
  async runInitialScraping() {
    ...
    const answer = await this.question(
      'Run initial data scraping now? This may take several minutes. (y/N): '
    );
    ...
    if (answer.toLowerCase() === 'y') {
      try {
        ...
        console.warn(
          '⚠️  Initial scraping failed, you can run it later with: npm run scrape-data'
        );
      }
    } else {
      console.log(
        'Skipping initial scraping. Run "npm run scrape-data" later.'
      );
    }
  }
